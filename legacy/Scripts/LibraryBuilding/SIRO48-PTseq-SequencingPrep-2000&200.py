...
# 样本来源板位 - 文库产物在POS11 Col 7-12 (用户手动放置)
source_plate = ('M2_POS11',7)

# 原位稀释：直接在POS11 Col 7-12的library产物孔内加T2 buffer
sample_dilution_place = ('M2_POS11',7)
...
# 为output_hybrid_pooling_info提供与全流程一致的变量名
filtered_samples = samples_from_csv
...
# 添加矿物油 - p1从POS24 Col3 Row1直接逐个加到POS20 Col7环化反应孔
p1_load_modified(tip_1000.load(1)[0])
for x in range(DNB_Num):
	p1_aspirate({"Position":"M2_POS24","Col":3,"Row":1,"PreAirVolume":8,"AspirateOffsetOfZ":0.8,"AspirateSpeed":10,"AspirateVolume":15,"PreAirSpeed":50,"DelayAfterAspirate":1,"PostAirSpeed":50,"PostAirVolume":0,"IfTrack":False,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80, "TipTouchTimes": 2, "TipTouchOffsetOfZ": 30, "TipTouchRangeOfX": 3, "TipTouchSpeed": 100})
	# Oil to cycling wells - Col 7, Row 1+x (每管覆盖15µL矿物油)
	p1_empty({"Position":"M2_POS20","Col":7,"Row":1+x,"EmptyOffsetOfZ":8,"EmptySpeed":30,"DelayAfterEmpty":0.5,"TipTouchTimes":0,"PostAirSpeed":50,"PostAirVolume":0,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80})
p1_unload_tips2({"Position":"M2_Trash","Col":None,"Row":None})
...
p8_load_modified(tip_300.load(1)[0])
# Step 1: 吸取E1 DNB聚合酶混合液I (大体积: 40×1.3×n)
