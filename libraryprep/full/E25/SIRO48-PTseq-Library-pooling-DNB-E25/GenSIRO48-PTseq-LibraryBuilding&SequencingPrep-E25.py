...
# -*- coding: utf-8 -*-
#####################################################################
# 脚本定位：GenSIRO48 E25 平台 PTseq 全流程脚本。
# 覆盖范围：从提取完成后的 cDNA 合成开始，完成建库、两轮磁珠纯化、Qubit dsDNA HS 定量、
#           E25 pooling 和 make DNB。
#####################################################################
# 建库液体体系、低通量分支、台面配置、POS11/POS14 对换和定量流程来自当前 library-only 脚本；
# pooling/DNB 段按 E25 sequencing prep 的实验逻辑衔接。
# Full E25 版的最终文库产物桥接到 POS13 Col7-12，随后进入 E25 pooling/DNB。
#
# 关键设计：
...
#   4. LA 纯化后使用新 PCR 板，LA 反应孔在 POS20 Col1-6；最终文库桥接到 POS13 Col7-12。
#   5. PCR 门管理规则：开盖前先开 PCR 门，盖回后立即关 PCR 门；定量后继续进入 E25 pooling/DNB。
...
# 新增阶段开始和结束时四块流程板必须回到以下 home，不改变后续建库的板位契约。
extraction_quant_plate_positions = {
	"purification_plate": "M2_POS16",
	"dye_deepwell": "M2_POS13",
	"waste_oil_pooling_plate": "M2_POS11",
	"quantification_tube_adapter": "M2_POS14",
}

def move_extraction_quant_plate(plate_name, target_pos):
	current_pos = extraction_quant_plate_positions[plate_name]
	if current_pos == target_pos:
		return
	for other_name, other_pos in extraction_quant_plate_positions.items():
		if other_name != plate_name and other_pos == target_pos:
			raise RuntimeError(f"Cannot move {plate_name} to occupied {target_pos}: {other_name} is present")
	transfer({"StartPosition":current_pos,"EndPosition":target_pos,"LoosenOffsetOfZ":0})
	extraction_quant_plate_positions[plate_name] = target_pos

def assert_extraction_quant_deck_restored():
	expected_positions = {
		"purification_plate": "M2_POS16",
		"dye_deepwell": "M2_POS13",
		"waste_oil_pooling_plate": "M2_POS11",
	...
	if extraction_quant_plate_positions != expected_positions:
		raise RuntimeError(f"Extraction quantification deck was not restored: {extraction_quant_plate_positions}")
...
def quantify_extraction_products():
	lang = get_lang()
	if lang == 1:
		report({"Phase":"提取产物定量","Step":"POS11 Col9-12 配制，POS14 Col7-10 定量","TaskType":"library","RemainingTime":None})
	elif lang == 2:
		report({"Phase":"Extraction quantification","Step":"Prepare in POS11 Col9-12; quantify in POS14 Col7-10","TaskType":"library","RemainingTime":None})

	if col_num > 4:
		sys.exit("E25 extraction quantification supports at most 32 samples in POS11 Col9-12")
	...
	for i in range(col_num - 1, -1, -1):
		...
		p8_empty_modified("M2_POS11", Row=1, Col=EXTRACTION_QUANT_MIX_START_COL+i, EmptyOffsetOfZ=3, TipTouchTimes=1)
	...
	for i in range(col_num):
		...
		p8_empty_modified("M2_POS11", Row=1, Col=EXTRACTION_QUANT_MIX_START_COL+i, EmptyOffsetOfZ=12)
		p8_mix({"Position":"M2_POS11","Col":EXTRACTION_QUANT_MIX_START_COL+i,"Row":1,"PreAirVolume":10,"MixTimes":2,"MixAspirateSpeed":100,"MixAspirateOffsetOfZ":1,"MixVolume":40,"MixDispenseOffsetOfZ":15,"MixDispenseSpeed":100,"DelayAfterMixLoop":0.5,"MixEmptyOffsetOfZ":5,"MixEmptySpeed":50,"PreAirSpeed":50,"DelayAfterMixAspirate":0.5,"DelayAfterMixDispense":0.5,"DelayAfterMixEmpty":0.5,"PostAirSpeed":50,"PostAirVolume":0,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80,"TipTouchTimes":0,"TipTouchOffsetOfZ":5,"TipTouchRangeOfX":1.2,"TipTouchSpeed":100})
		p8_unload_modified(sample_tips[i])

	# POS16 纯化板暂存 POS23，POS11 深孔板进入 POS16，执行与建库定量一致的双向震荡。
	move_extraction_quant_plate("purification_plate", "M2_POS23")
	move_extraction_quant_plate("waste_oil_pooling_plate", "M2_POS16")
	...
	move_extraction_quant_plate("waste_oil_pooling_plate", "M2_POS11")
	move_extraction_quant_plate("purification_plate", "M2_POS16")

	# 腾空 POS13，把 POS14 定量管移入操作位；从 POS11 每孔分 4 次转移 50 uL。
	move_extraction_quant_plate("purification_plate", "M2_POS23")
	move_extraction_quant_plate("dye_deepwell", "M2_POS16")
	move_extraction_quant_plate("quantification_tube_adapter", "M2_POS13")
	for i in range(col_num):
		p8_load_modified(sample_tips[i])
		for x in range(4):
			p8_aspirate_modified("M2_POS11", Row=1, Col=EXTRACTION_QUANT_MIX_START_COL+i, PreAirVolume=5, AspirateVolume=50, AspirateOffsetOfZ=1, PostAirVolume=3, IfTrack=True)
	...
	move_extraction_quant_plate("purification_plate", "M2_POS16")
	assert_extraction_quant_deck_restored()
...
- Columns 7-12 (Concentrated Library): 最终洗脱的目的地（Full E25: 产物在POS13）
...
# 双选产物位置 - Full E25: 产物回收到 POS13 Col7-12，避免占用 POS20 DNB 反应位。
...
# 样本来源起始位置 - Full E25: 从 POS13 Col7-12 取定量样本。
...
for i in range(col_num):
	...
	p8_aspirate_modified(source_plate[0], 1, source_plate[1]+i, 2.2, AspirateSpeed=2, AspirateOffsetOfZ=0.3, IfTrack=True)
...
# E25 DNB反应孔内的文库总投入量和体积。
target_dna_ng = 15.84
target_pooling_volume = 20
max_intermediate_concentrate_times = target_pooling_volume * 8 / min_sample_volume
# 质控浓度
...
# pooling 产物位置：POS14/POS11 对换后，pooling 汇集管固定在 M2_POS11 Col7。
target_tube_loc = [('M2_POS11',7,i) for i in range(1,9)]
# E25 DNB 反应位置：pooling产物直接进入POS20 Col8 Row1-6。
...
class Sample:
	def __init__(self, SampleWellPosition,SampleWellColumn ,SampleWellRow , Concentration,DilutingWellPosition,DilutingWellColumn,DilutingWellRow,DilutingSampleVolume=0, DilutingBufferVolume=0, sample_id = ""):
		...
		self.corrected_concentration = Concentration
		self.base_corrected_concentration = Concentration
		self.data_amount = 1
...
def group_samples_fixed_capacity_by_barcode(samples):
	...
	if len(groups) > len(target_dnb_loc_list):
		raise Exception(f"E25全流程当前最多支持 {len(target_dnb_loc_list)} 个DNB，barcode避让后需要 {len(groups)} 个DNB")
...
def apply_dilution_state(sample):
	...
	sample.corrected_concentration = sample.base_corrected_concentration / sample.dilution_type
	sample.Concentration = sample.corrected_concentration
	sample.pooling_concentration = sample.corrected_concentration / sample.data_amount

def calculate_pooling_plan(cur_samples):
	concentrate_times = 1
	...
	return concentrate_times, water_volume, target_transfer_volume, target_volume_list

def promote_dilution(sample):
	if sample.dilution_type == 1:
		sample.dilution_type = 8
	elif sample.dilution_type == 8:
		sample.dilution_type = 64
	else:
		return False
	apply_dilution_state(sample)
	return True

def is_pooling_plan_valid(cur_samples, plan):
	concentrate_times, water_volume, target_transfer_volume, target_volume_list = plan
	if water_volume < 0 or target_transfer_volume < min_sample_volume:
		return False
	if min(target_volume_list) < min_sample_volume or max(target_volume_list) > max_sample_volume:
		return False
	if concentrate_times > max_intermediate_concentrate_times:
		return False
	return True
...
def mark_dilution_samples(groups):
	for group in groups:
		for sample in group:
			sample.dilution_type = 1
			apply_dilution_state(sample)
		min_pooling_concentration = min(sample.pooling_concentration for sample in group)
		max_pooling_concentration = max(sample.pooling_concentration for sample in group)
		if max_pooling_concentration / min_pooling_concentration > 8:
			for sample in group:
				if sample.pooling_concentration >= min_pooling_concentration * 64:
					sample.dilution_type = 64
				elif sample.pooling_concentration >= min_pooling_concentration * 8:
					sample.dilution_type = 8
				apply_dilution_state(sample)
		for _ in range(len(group) * 2 + 1):
			plan = calculate_pooling_plan(group)
			if is_pooling_plan_valid(group, plan):
				break
			target_sample = most_limiting_sample(group)
			if not promote_dilution(target_sample):
				raise Exception(f"DNB组 {target_sample.group_idx} 中样本 {target_sample.sample_id} 64x稀释后仍无法满足E25 pooling体积要求，请降低文库浓度或拆分DNB")
		else:
			raise Exception("E25 pooling体积可行性检查未收敛，请检查输入浓度")

mark_dilution_samples(dnb_list)

def get_sample_volume(cur_samples):
	concentrate_times, water_volume, target_transfer_volume, target_volume_list = calculate_pooling_plan(cur_samples)
...
def preflight_pooling_plan(groups, temp, water_volume_list):
	...
	for group_index, group in enumerate(groups):
		...
		if concentrate_times >= 8:
			...
			target_transfer_volume = target_pooling_volume
		if water_volume < 0:
			errors.append(f"DNB组 {group_index+1} 补水体积为 {water_volume:.2f} uL")
		if target_transfer_volume < min_sample_volume:
			errors.append(f"DNB组 {group_index+1} pooling产物转入DNB孔体积为 {target_transfer_volume:.2f} uL")
		for sample in group:
			if sample.DilutingSampleVolume < min_sample_volume or sample.DilutingSampleVolume > max_sample_volume:
				errors.append(f"样本 {sample.sample_id} 取样体积 {sample.DilutingSampleVolume:.2f} uL 超出 {min_sample_volume}-{max_sample_volume} uL")
	if errors:
		raise Exception("E25 pooling preflight失败: " + "; ".join(errors))

temp = [get_sample_volume(each) for each in dnb_list]
	...
	with open(output_file_path, 'w', encoding='utf-8') as f:
		...
		for i, group in enumerate(samples):
			...
			for sample in group:
				...
				f.write(f"{sample.sample_id},{cur_pooling_id},{formated_vol},{sample.dilution_type},{concentrate_times},{conc}\n")
...
# 第一步：将 POS11（原 POS14 pooling 汇集板）移到 POS23，利用 POS23 作为可被 P1/P8 访问的操作位。
...
for i,poolings in enumerate(temp):
	...
	for sample in samples:
		...
		if not sample.NeedDilution:
			...
			p8_mix({"Position":sample.DilutingWellPosition,"Col":sample.DilutingWellColumn,"Row":sample.DilutingWellRow,"PreAirVolume":10,"MixTimes":5,"MixAspirateSpeed":100,"MixAspirateOffsetOfZ":0.5,"MixVolume":mix_volume,"MixDispenseOffsetOfZ":10,"MixDispenseSpeed":100,"DelayAfterMixLoop":0.5,"MixEmptyOffsetOfZ":10,"MixEmptySpeed":100,"PreAirSpeed":50,"DelayAfterMixAspirate":0.5,"DelayAfterMixDispense":0.5,"DelayAfterMixEmpty":0.5,"PostAirSpeed":50,"PostAirVolume":0,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80,"TipTouchTimes":1,"TipTouchOffsetOfZ":5,"TipTouchRangeOfX":1.2,"TipTouchSpeed":100})
...
for i in range(target_dnb_num):
	...
	p8_mix({"Position":target_dnb_loc_list[i][0],"Col":target_dnb_loc_list[i][1],"Row":target_dnb_loc_list[i][2],"PreAirVolume":0,"MixTimes":3,"MixAspirateSpeed":100,"MixAspirateOffsetOfZ":0.5,"MixVolume":15,"MixDispenseOffsetOfZ":8,"MixDispenseSpeed":100,"DelayAfterMixLoop":1,"MixEmptyOffsetOfZ":5,"MixEmptySpeed":200,"PreAirSpeed":50,"DelayAfterMixAspirate":0.5,"DelayAfterMixDispense":0.5,"DelayAfterMixEmpty":0.5,"PostAirSpeed":50,"PostAirVolume":0,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80, "TipTouchTimes": 0, "TipTouchOffsetOfZ": 5, "TipTouchRangeOfX": 1.2, "TipTouchSpeed": 100})
	p8_empty_modified(target_dnb_loc_list[i][0],target_dnb_loc_list[i][2],target_dnb_loc_list[i][1])
	p8_unload_tips({"Position":"M2_Trash","Col":None,"Row":None})

# 第七步：将 POS23 上的 pooling 汇集板移回 POS11；PCR 盖保持打开，后续 E25 DNB 需要继续操作 POS20。
...
# Mix II 源管须预置 4*(DNB_Num+0.5) uL Mix II；Mix I 来源装量另含 6 uL 管底冗余。
...
def blockD3():
	lang = get_lang()
	if lang == 1:
		report({"Phase":"DNB制备完成","Step":"4℃保温中","TaskType":"library","RemainingTime":None})
	elif lang == 2:
		report({"Phase":"DNB preparation complete","Step":"Holding at 4°C","TaskType":"library","RemainingTime":None})
