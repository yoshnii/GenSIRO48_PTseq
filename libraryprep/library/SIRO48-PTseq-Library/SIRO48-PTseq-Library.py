...
def clear_pos13_for_transit():
	"""Park the POS13 deepwell at POS16 and return whether POS16 had to be cleared."""
	...
	elif purification_plate_pos != "M2_POS23":
		raise RuntimeError("Purification plate is not at POS16/POS23")
...
def swap_pos20_and_pos9_pcr_plates():
	"""Move the used POS20 PCR plate to POS9 and the fresh POS9 plate to POS20 via POS13."""
	moved_purification_plate = clear_pos13_for_transit()
...
# LA/PCR Master Mix 现配制在 POS17 C2R3（T8 2.0 mL 管，T8 手工预置在此管内）。
# 该 mixing tube 单独设定 collection 冗余，不再与 TA 共用 TA_LA_MIX_TUBE_DEAD_VOLUME，
# 避免改动 LA 冗余时连带影响 TA Master Mix。冗余体现在总 mix 体积里，再按 20:1:9 分摊到 T7/T8/T2。
...
# 从 POS8 后半板 Col7-12 均一化产物转移 14 uL 到 POS20 Col1-6，并与 T1 在孔内混匀。
col_num = (sample_num+7)//8
column_num = col_num
for i in range(col_num):
...
if not low_throughput_p1_direct_col11:
	...
	p1_load_modified(tip_1000.load(1)[0])
	for i in range(8):
		# LA/PCR Master Mix 在 POS17 C2R3（Col2 Row3）混匀后直接预分装到 POS7 Col11。
...
if low_throughput_p1_direct_col11:
	...
	for tip_index, (col_index, row) in enumerate(active_sample_wells(SampleCount)):
		...
		# 从 POS17 C2R3 mixing tube 直接吸 30 uL（Row=3, Col=2）分装到 POS23 干燥磁珠孔。
...
dye_mix_plate[0] = dye_deepwell_pos
