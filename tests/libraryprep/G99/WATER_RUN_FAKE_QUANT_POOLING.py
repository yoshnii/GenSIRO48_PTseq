...
# WATER-RUN DELETE-PCR-BLOCKS VERSION: PCR program blocks and their waits are removed.
# PCR door open/close calls and PCR lid transfers are preserved.
'''=====================================以上为样本信息读取建库仪====================================='''
...
for i in range(col_num):
	...
	# FIX (Task 5): DelayAfterAspirate 0.5→3s to let liquid settle in tip, reduces uneven aspiration across 8 channels
	p8_aspirate({"Position":"M2_POS7","Col":11,"Row":1,"PreAirVolume":5,"AspirateOffsetOfZ":0.5,"AspirateSpeed":50,"AspirateVolume":30,"PreAirSpeed":50,"DelayAfterAspirate":3,"TipTouchTimes":0,"PostAirSpeed":50,"PostAirVolume":5,"IfTrack":False,"FirstSegmentSpeed":100,"SpeedChangeOffsetOfZ":0,"SecondSegmentSpeed":80})
...
pcr_close_door()  # WATER-RUN: close PCR door retained; PCR program block removed
...
# v12: LA 纯化晾干延时 5 min (8→5 min, 回退至 SOP 允许下限)
delay({"Duration": 300})
...
# 单个定量管位置，板列行
quantification_tubes = [(quantification_tube_loc[0],quantification_tube_loc[1]+i//8,1 + i%8) for i in range(sample_num)]
#=================================== WATER-RUN FAKE QUANT INJECTION ===================================
# Replaces the real Qubit hardware block (dye dispense, shaker swap, sample-to-quant-tube
# transfer, and live concentration readout). Hardcodes 8 fake concentrations so the
# downstream pooling/DNB logic can be exercised without real reagents.
#
# REQUIRES: SampleCount = 8 (this fake list has exactly 8 values).
# Sample 1 is deliberately set 200 ng/µL (>10× higher than the rest) to trigger the
# NeedDilution branch in the pooling code (max/min > 8 threshold).
#======================================================================================================
fake_concentration_list = [
	200.0,  # Sample 1 — triggers NeedDilution path in pooling logic
	20.0,
	22.0,
	24.0,
	21.0,
...
concentration_list = list(fake_concentration_list)
if len(concentration_list) != sample_num:
	raise ValueError("Fake quant water-run requires SampleCount=8 and exactly 8 fake concentration values.")
print(f"[WATER_RUN_FAKE_QUANT] injected fake concentration_list = {concentration_list}")
...
# [v11] 独立稀释：从POS13 Col 7-12 library产物取2µL到POS8 Col 7-12 PCR板稀释孔
...
# [v11] 独立 Step 3 mix 块已删除：改独立稀释孔后 sample 尚未进入稀释孔，mix 并入 Step 5 pool 循环
