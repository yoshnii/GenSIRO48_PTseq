...
### ====================== 水跑确认弹窗函数 ======================
def water_run_confirm(group_id, group_name, details):
	"""水跑确认弹窗 - 返回 '执行' 或 '跳过', 中止则退出脚本"""
	a = dialog_textbox({
		"Title": "[水跑确认] " + group_id + " " + group_name,
		"Timeout": "02:00:00",
		"Parameters": [
			{"Name": "操作", "Value": "执行", "Notes": details + "\n\n请输入: 执行 / 跳过 / 中止"}
		]
	})
	action = a["操作"].strip()
	if action == "中止":
		report({"Phase": "水跑中止", "Step": "用户在 " + group_id + " 中止了脚本", "TaskType": "library", "RemainingTime": None})
		home()
		sys.exit(0)
	return action

### ====================== 共享变量预初始化 ======================
# 这些变量被多个功能组使用，预先计算以支持跳过单个功能组
...
### ===== 水跑确认 E1: 磁珠混匀+分装 =====
_wr_action = water_run_confirm("E1", "磁珠混匀+分装", "来源: POS10 → POS5/9/13 Col1,7\n试剂: T1磁珠\nTarget: 20µL/反应\n实际: 20µL(≤8样本) 或 20×1.3µL/管(>8样本)")
if _wr_action == "执行":
	#=========================================混匀磁珠=====================================================
	lang=get_lang()
	if lang==1: #
...
### ===== 水跑确认 E2: 添加异丙醇 =====
_wr_action = water_run_confirm("E2", "添加异丙醇", "来源: POS4 Col2 → POS5/9/13 Col1,7\n试剂: 异丙醇\nTarget: 230µL\n实际: 230µL")
if _wr_action == "执行":
	#================================添加异丙醇=====================================================
	lang=get_lang()
	if lang==1: #
...
### ===== 水跑确认 E3: 分装回溶液 =====
_wr_action = water_run_confirm("E3", "分装回溶液", "来源: POS14 → POS5/9/13 Col6,12\n试剂: 洗脱液(回溶液)\nTarget: 25µL(说明书)\n实际: 45-50µL")
if _wr_action == "执行":
	#=========================================分装回溶液=====================================================
	lang=get_lang()
	if lang==1: #
...
### ===== 水跑确认 E4: 预分装样本 =====
_wr_action = water_run_confirm("E4", "预分装样本", "来源: Lane1/Lane2 → POS2\n试剂: 样本上清\nTarget: 425µL\n实际: 425µL")
if _wr_action == "执行":
	target_col_list = [1,7]
	target_row_list = [i+1 for i in range(8)]
	#=========================================预分样本=====================================================
...
### ===== 水跑确认 E5: 分装洗液A =====
_wr_action = water_run_confirm("E5", "分装洗液A", "来源: POS4 Col3 → POS5/9/13 Col3,9\n试剂: 洗液A\nTarget: 350µL(说明书)\n实际: 500µL")
if _wr_action == "执行":
	#=========================================分装洗液1=====================================================
	lang=get_lang()
	if lang==1: #
...
### ===== 水跑确认 E6: 分装洗液B =====
_wr_action = water_run_confirm("E6", "分装洗液B", "来源: POS4 Col4-5 → POS5/9/13 Col4-5,10-11\n试剂: 洗液B\nTarget: 640µL(说明书)\n实际: 600µL×2列")
if _wr_action == "执行":
	#=========================================分装洗液2=====================================================
	lang=get_lang()
	if lang==1: #
...
### ===== 水跑确认 E7: 添加样本到深孔板 =====
_wr_action = water_run_confirm("E7", "添加样本到深孔板", "来源: POS2 → POS5/9/13 Col1,7\n试剂: 预分装样本\nTarget: 425µL\n实际: 425µL")
if _wr_action == "执行":
	#=========================================添加样本到深孔板=====================================================
...
### ===== 水跑确认 E8: 磁棒提取+产物回收 =====
_wr_action = water_run_confirm("E8", "磁棒提取+产物回收", "来源: POS5/9/13 Col6 → T1管\n试剂: 核酸产物(磁棒提取)\nTarget: 45µL\n实际: 45µL")
if _wr_action == "执行":
	#==========================================磁棒提取===============================================
	lang=get_lang()
	if lang==1: #
